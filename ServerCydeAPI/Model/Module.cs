...
namespace ServerCydeAPI
{
    public class Module : IHttpModule
    {
        ...
        {
            HttpContext.Current.Response.Headers.Set("Server", "OpenBox");
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        long siteid = web.UrlParts[1].ToLong(0);
                        ...
                        errorMsg += "<h3>Session</h3> " + (context.Request.Cookies["Identity" + siteid] != null ? context.Request.Cookies["Identity" + siteid].Else("na") : "na");
                        ...
                        //       && !errorMsg.Contains("-Not"))
                        {
                            ...
                            if (context.Items["HandlerType"] != null && (Handler.ContentType)context.Items["HandlerType"] == Handler.ContentType.JSON )
                            { 
                                    
                                    web.val.ErrorMsg = output;
                                    //web.Write(web.ToJSON());

                                    if (web.Form["ServerCydeRequestType"].Like("failsafe"))
                                    {
                                        HttpRuntime.Cache.Add(guid, web.ToJSON(), null, DateTime.Now.AddMinutes(1), System.Web.Caching.Cache.NoSlidingExpiration, System.Web.Caching.CacheItemPriority.BelowNormal, null);
                                        context.Response.Redirect("http://" + context.Request.UrlReferrer.DnsSafeHost + "/servercyde/?id=" + guid);
                                    }
                                    else
                                    {
                                        context.Response.ContentType = "application/json";
                                        context.Response.Write(web.ToJSON());
                                    }
                                   
                            }
                            else
                            {
                                context.Response.Write("hostname: " + context.Request.Url.Host + (context.Request.Url.Host.Like("localhost") ? ":" + context.Request.Url.Port : ""));
